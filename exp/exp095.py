...
def main_process():
    ...
    for m in [0.2]:
        cfg = Config(experiment_name=f"nlp_model=bert-multi/cnn_model=swin_base_patch4_window12_384/m={m}",
                     m=m)
    ...
    for m in [0.2, 0.3]:
        cfg = Config(experiment_name=f"nlp_model=bert-multi/cnn_model=vit_base_patch16_384/m={m}",
                     m=m)
        cfg.metric_layer_params["in_features"] = 2048
        cfg.metric_layer_params["m"] = m
        main(cfg)
