...
DEBUG = False
EXPERIMENT_NAME = "transformer_tune(without layernorm)"
...
class Config:
    ...
    scheduler_params = {"T_0": 15000, "T_mult": 1, "eta_min": 1e-6}
...
class TransformerEncoderLayerWithoutLayerNorm(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        super(TransformerEncoderLayerWithoutLayerNorm, self).__init__()
    ...
    def __setstate__(self, state):
        ...
        super(TransformerEncoderLayerWithoutLayerNorm, self).__setstate__(state)
    ...
    def forward(self, src: torch.Tensor, src_mask: Optional[torch.Tensor] = None, src_key_padding_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # src = self.norm1(src.transpose(2, 1)).transpose(1, 2)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = src + self.dropout2(src2)
        # src = self.norm2(src.transpose(2, 1)).transpose(1, 2)
        return src
...
class ShopeeNet(nn.Module):
    def __init__(self,
        ...
        encoder_layer = TransformerEncoderLayerWithoutLayerNorm(d_model=self.bert.hidden_size,
...
def main_process():
    """
    for transformer_lr in [1e-3, 1e-4, 1e-5]:
        config = Config()
    ...
    for dropout_transformer in [0.1, 0.5]:
        config = Config()
