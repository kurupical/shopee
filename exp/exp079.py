...
EXPERIMENT_NAME = "exp079: 3loss+CNN(max/avgpool)"
DEBUG = False
...
class ShopeeNet(nn.Module):
    def __init__(self,
        ...
        self.cnn = create_model(config.model_name,
                                ...
                                num_classes=0)
        self.cnn_bn = nn.BatchNorm1d(self.cnn.num_features*2)
        self.cnn_fc = nn.Linear(self.cnn.num_features*2, config.linear_out)
    ...
    def forward(self, X_image, input_ids, attention_mask, label=None):
        ...
        img = torch.cat([F.adaptive_avg_pool2d(img, (1, 1)).squeeze(2).squeeze(2),
                         F.adaptive_max_pool2d(img, (1, 1)).squeeze(2).squeeze(2)], dim=1)
        img = self.cnn_bn(img)
