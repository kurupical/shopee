...
DEBUG = False
EXPERIMENT_NAME = "transformer_text_cnn"
...
class Config:
    ...
    dropout_bert_stack: float = 0.5
    dropout_transformer: float = 0.2
    dropout_cnn_fc: float = 0.5
...
class ShopeeNet(nn.Module):
    def __init__(self,
        ...
        encoder_layer_cnn = nn.TransformerEncoderLayer(d_model=self.bert.hidden_size,
                                                       nhead=config.transformer_n_heads,
                                                       dropout=config.dropout_transformer)
        encoder_layer_concat = nn.TransformerEncoderLayer(d_model=self.bert.hidden_size,
                                                          nhead=config.transformer_n_heads,
                                                          dropout=config.dropout_transformer)

        self.transformer_cnn = nn.TransformerEncoder(encoder_layer=encoder_layer_cnn,
                                                     num_layers=config.transformer_num_layers)
        self.transformer_concat = nn.TransformerEncoder(encoder_layer=encoder_layer_concat,
                                                        num_layers=config.transformer_num_layers)
    ...
    def forward(self, X_image, input_ids, attention_mask, label=None):
        ...
        x = self.transformer_cnn(x.permute(1, 0, 2)).permute(1, 0, 2)
        text = self.bert(input_ids, attention_mask)

        x = torch.cat([F.normalize(text), F.normalize(x)], dim=1)
        x = self.transformer_concat(x.permute(1, 0, 2)).permute(1, 0, 2)
        x = x.mean(dim=1)
        ret = self.fc(x)
...
def main(config, fold=0):
    ...
    try:
        ...
        optimizer = config.optimizer(params=[{"params": model.bert.parameters(), "lr": config.bert_lr},
                                             ...
                                             {"params": model.transformer_cnn.parameters(), "lr": config.transformer_lr},
                                             {"params": model.transformer_concat.parameters(), "lr": config.transformer_lr},
