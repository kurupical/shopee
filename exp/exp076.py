...
DEBUG = False
EXPERIMENT_NAME = "transformer_hidden_dimension_wise"
...
class ShopeeNet(nn.Module):
    def __init__(self,
        ...
        n_feat_concat = self.bert.hidden_size + self.cnn.num_features // 4
    ...
    def forward(self, X_image, input_ids, attention_mask, label=None):
        x = self.cnn.forward_features(X_image).reshape(len(X_image), self.cnn.num_features, -1).transpose(1, 2)
        text = self.bert(input_ids, attention_mask)

        x = x[:, ::2, ::4] + x[:, ::2, 1::4] + x[:, ::2, 2::4] + x[:, ::2, 3::4] + \
            x[:, 1::2, ::4] + x[:, 1::2, 1::4] + x[:, 1::2, 2::4] + x[:, 1::2, 3::4]
        x = torch.cat([F.normalize(text), F.normalize(x)], dim=2)
