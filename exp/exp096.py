...
class BertModule(nn.Module):
    def __init__(self,
        ...
        self.bert_bn_hidden = nn.BatchNorm1d(self.hidden_size)
        self.bert_bn_cls = nn.BatchNorm1d(self.hidden_size)
    ...
    def forward(self, input_ids, attention_mask):
        if self.config.nlp_model_name == "cahya/distilbert-base-indonesian":
            ...
            text_hidden = text.mean(dim=1)
            text_hidden = self.bert_bn_hidden(text_hidden)
            text_hidden = self.dropout_nlp(text_hidden)

            text_cls = text[:, 0, :]
            text_cls = self.bert_bn_hidden(text_cls)
            text_cls = self.dropout_nlp(text_cls)

        else:
            text = self.bert(input_ids=input_ids, attention_mask=attention_mask, output_hidden_states=True)[2]

            text_hidden = torch.stack([self.dropout_stack(x) for x in text[-4:]]).mean(dim=0)
            text_hidden = torch.sum(
                text_hidden * attention_mask.unsqueeze(-1), dim=1, keepdim=False
            )
            text_hidden = text_hidden / torch.sum(attention_mask, dim=-1, keepdim=True)
            text_hidden = self.bert_bn_hidden(text_hidden)
            text_hidden = self.dropout_nlp(text_hidden)

            text_cls = torch.stack([self.dropout_stack(x[:, 0, :]) for x in text[-4:]]).mean(dim=0)
            text_cls = self.bert_bn_cls(text_cls)
            text_cls = self.dropout_nlp(text_cls)
...
class ShopeeNet(nn.Module):
    def __init__(self,
        ...
        self.bert = BertModule(bert=bert,
                               config=config)
        self.bert_cls_fc = nn.Linear(self.bert.hidden_size, config.linear_out)
        self.bert_hidden_fc = nn.Linear(self.bert.hidden_size, config.linear_out)
        ...
        n_feat_concat = config.linear_out*3
        ...
        self.fc_bert_hidden_out = nn.Sequential(
            nn.Dropout(config.dropout_nlp),
            nn.Linear(config.linear_out, config.linear_out),
            nn.BatchNorm1d(config.linear_out)
        )
        self.fc_bert_cls_out = nn.Sequential(
            nn.Dropout(config.dropout_nlp),
            nn.Linear(config.linear_out, config.linear_out),
            nn.BatchNorm1d(config.linear_out)
        )
    ...
    def forward(self, X_image, input_ids, attention_mask, label=None):
        ...
        text_hidden, text_cls = self.bert(input_ids, attention_mask)
        text_hidden = self.bert_hidden_fc(text_hidden)
        text_cls = self.bert_cls_fc(text_cls)

        x = torch.cat([img, text_hidden, text_cls], dim=1)
        ret = self.fc(x)

        ret_img = self.fc_cnn_out(img)
        ret_text_hidden = self.fc_bert_hidden_out(text_hidden)
        ret_text_cls = self.fc_bert_cls_out(text_cls)

        if label is not None:
            x = self.final(ret, label)
            img = self.final(ret_img, label)
            text_hidden = self.final(ret_text_hidden, label)
            text_cls = self.final(ret_text_cls, label)
            return x, img, text_hidden, text_cls, ret, ret_img, ret_text_hidden, ret_text_cls
        else:
            return ret
...
def train_fn(dataloader, model, criterion, optimizer, device, scheduler, epoch, ):
    ...
    loss_texts_hidden = AverageMeter()
    loss_texts_cls = AverageMeter()
    ...
    for bi, d in tk0:
        ...
        output, img, text_hidden, text_cls, _, _, _, _ = model(images, input_ids, attention_mask, targets)

        loss_output = criterion(output, targets)
        loss_img = criterion(img, targets)
        loss_text_hidden = criterion(text_hidden, targets)
        loss_text_cls = criterion(text_cls, targets)
        loss = loss_output + loss_img + loss_text_hidden + loss_text_cls
        loss.backward()
        optimizer.step()
        ...
        loss_texts_hidden.update(loss_text_hidden.detach().item(), batch_size)
        loss_texts_cls.update(loss_text_cls.detach().item(), batch_size)
        tk0.set_postfix(LossOut=loss_outputs.avg,
                        LossImg=loss_imgs.avg,
                        LossTHid=loss_texts_hidden.avg,
                        LossTCLS=loss_texts_cls.avg,
...
def eval_fn(data_loader, model, criterion, device, df_val, epoch, output_dir):
    ...
    with torch.no_grad():
        for bi, d in tk0:
            ...
            output, _, _, _, feature, img, text_hidden, text_cls = model(images, input_ids, attention_mask, targets)
            ...
            all_texts_hidden.extend(text_hidden.detach().cpu().numpy())
            all_texts_cls.extend(text_cls.detach().cpu().numpy())
            all_targets.extend(targets.detach().cpu().numpy())

    all_features = np.array(all_features, dtype=np.float32)
    all_images = np.array(all_images, dtype=np.float32)
    all_texts_hidden = np.array(all_texts_hidden, dtype=np.float32)
    all_texts_cls = np.array(all_texts_cls, dtype=np.float32)
    ...
    np.save(f"{output_dir}/embeddings_texts_hidden_epoch{epoch}.npy", all_texts_hidden)
    np.save(f"{output_dir}/embeddings_texts_hidden_epoch{epoch}.npy", all_texts_cls)

    del all_images, all_texts_hidden, all_texts_cls
    gc.collect()
...
def main(config, fold=0):
    ...
    try:
        ...
        optimizer = config.optimizer(params=[{"params": model.bert.parameters(), "lr": config.bert_lr},
                                             {"params": model.bert_cls_fc.parameters(), "lr": config.fc_lr},
                                             {"params": model.bert_hidden_fc.parameters(), "lr": config.fc_lr},
                                             ...
                                             {"params": model.fc_bert_hidden_out.parameters(), "lr": config.fc_lr},
                                             {"params": model.fc_bert_cls_out.parameters(), "lr": config.fc_lr},
...
def main_process():
    ...
    cfg = Config(experiment_name=f"[CLS4*hidden_mean4+imgpool]nlp_model=bert-multi/cnn_model=vit_base_patch16_384/")
    main(cfg)
