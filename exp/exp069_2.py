...
EXPERIMENT_NAME = "triplet_loss_between_text_and_image(miner fix)"
DEBUG = False
...
def train_fn(dataloader, model, criterion, criterion_pair, miner, optimizer, device, scheduler, epoch, ):
    ...
    for bi, d in tk0:
        ...
        hard_pairs = miner(embeddings=torch.cat([img, text]),
                           labels=torch.cat([targets, targets]),
                           ref_emb=torch.cat([img, text]),
                           ref_labels=torch.cat([targets, targets]))
        loss2 = criterion_pair(torch.cat([img, text]), torch.cat([targets, targets]), hard_pairs)
