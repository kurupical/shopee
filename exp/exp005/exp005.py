...
def eval_fn(data_loader, model, criterion, device, df_val):
    ...
    best_score, best_th = get_best_neighbors(df=df_val, embeddings=all_features)
...
def main(config):
    ...
    train_dataset = ShopeeDataset(df=df,
                                  ...
                                  tokenizer=tokenizer)
    val_dataset = ShopeeDataset(df=df,
    ...
    for epoch in range(config.epochs):
        ...
        valid_loss, best_score, best_threshold = eval_fn(val_loader, model, criterion, device, df)
        ...
        df.to_csv(f"{output_dir}/df_val.csv", index=False)
...
if __name__ == "__main__":
    ...
    for out in [128, 256, 512, 1024]:
        config = Config()
        config.linear_out = out
        main(config)
