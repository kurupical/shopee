...
class ShopeeNet(nn.Module):
    def __init__(self,
        ...
        self.positional_encoding = nn.Embedding(128+256, 768)
    ...
    def forward(self, X_image, input_ids, attention_mask, label=None):
        ...
        pos_encoding = self.positional_encoding(torch.arange(128+256).repeat(len(x), 1).to("cuda"))
        x = x + pos_encoding
...
def get_best_neighbors(embeddings, df, epoch, output_dir):
    ...
    for th in np.arange(20, 60, 1).tolist():
        preds = []
...
def main_process():
    ...
    for lr in [1e-5]:
        cfg = Config(experiment_name=f"=efficientnet_v3/position_encoding/transformer_lr={lr}")
