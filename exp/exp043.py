...
class ShopeeNet(nn.Module):
    ...
    def forward(self, X_image, input_ids, attention_mask, label=None):
        ...
        text = self.fc_bert(text)
        text = self.dropout_fc(text)
...
def main(config, fold=0):
    ...
    try:
        ...
        optimizer = config.optimizer(params=[{"params": model.bert.parameters(), "lr": config.bert_lr},
                                             ...
                                             {"params": model.fc_cnn.parameters(), "lr": config.fc_lr},
                                             {"params": model.fc_bert.parameters(), "lr": config.fc_lr},
...
def main_process():
    ...
    for dropout_fc in [0, 0.2, 0.5]:
        config = Config()
        config.dropout_fc = dropout_fc
        main(config)
