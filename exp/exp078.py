...
DEBUG = False
EXPERIMENT_NAME = "transformer_head2"
...
def main_process():
    ...
    """
    for transformer_lr in [1e-5, 0.5e-5]:
        config = Config()
    ...
    for m in [0, 0.25, 0.75]:
        config = Config()
        config.m = m
        main(config)

    for fc_lr in [1e-3, 1e-4]:
        config = Config()
        config.fc_lr = fc_lr
        main(config)

    for dropout_stack in [0]:
        config = Config()
    ...
    for dropout_cnn_fc in [0]:
        config = Config()
        config.dropout_cnn = dropout_cnn_fc
        main(config)
