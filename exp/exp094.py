...
def get_best_neighbors(embeddings, df, epoch, output_dir):
    ...
    for th in np.arange(20, 50, 0.5).tolist():
        preds = []
...
def main_process():
    ...
    """
    for linear_out in [2048+1024]:
        cfg = Config(experiment_name=f"cnn_model=vit_base_patch16_384/linear_out={linear_out}",
                     linear_out=linear_out)
        cfg.metric_layer_params["in_features"] = linear_out
        main(cfg)

    cfg = Config(experiment_name=f"cnn_model=swin_base_patch4_window12_384/linear_out={linear_out}",
                 linear_out=2048,
                 model_name="swin_base_patch4_window12_384")
    cfg.metric_layer_params["in_features"] = 2048
    main(cfg)


    # for lr in [1e-5, 4e-5, 16e-5, 64e-5]:
    for lr in [4e-5, 16e-5, 64e-5]:
        cfg = Config(experiment_name=f"cnn_model=vit_base_patch16_384/distilbert/bert_lr={lr}",
                     bert_lr=lr,
                     nlp_model_name="cahya/distilbert-base-indonesian")
        cfg.metric_layer_params["in_features"] = 2048
        main(cfg)
    """

    for m in [0.2, 0.4, 0.6]:
        cfg = Config(experiment_name=f"cnn_model=vit_base_patch16_384/m={m}",
                     m=m)
        cfg.metric_layer_params["in_features"] = 2048
